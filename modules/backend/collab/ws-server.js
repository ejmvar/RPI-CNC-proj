...
function createWsCollabServer(collabServer, opts = {}) {
  ...
  wss.on('connection', (socket) => {
    ...
    socket.on('message', (raw) => {
      ...
      try {
        if (action === 'create') {
          const { sessionId, initialState, permissions } = msg;
          // if clientId present, store owner info
          const owner = msg.clientId || null;
          const state = Object.assign({}, initialState || {}, owner ? { owner } : {});
          const opts = { owner, permissions: permissions || { mode: 'open' } };
          collabServer.createSession(sessionId, state, opts);
          ...
          const { sessionId, patch, clientId } = msg;
          collabServer.updateSession(sessionId, patch || {}, clientId);
          ...
          const { sessionId, permissions, clientId } = msg;
          collabServer.setPermissions(sessionId, permissions, clientId);
          const s = collabServer.getSession(sessionId);
          broadcastToSession(sessionId, {
  ...
  collabServer.on('sessionUpdated', ({ sessionId }) => {
