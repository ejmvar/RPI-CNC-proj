...
describe('gcode optimizer', () => {
  test('removes redundant movement lines', () => {
    ...
    const code = `import('./${path.relative(process.cwd(), filepath).replace(/\\/g, '/')}').then(m=>console.log(m.optimizeGCode('G1 X0 Y0 Z0\\nG1 X0 Y0 Z0\\nG1 X5 Y5 Z-1'))).catch(e=>{console.error(e);process.exit(2)})`;
    ...
    expect(out.length).toBe(2); // one redundant line removed
  });
});
