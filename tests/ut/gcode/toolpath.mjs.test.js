...
describe('G-code toolpath ES module (unit)', () => {
  test('parseGCodeToPoints converts G-Code to point list', () => {
    ...
    const script = `import('./${path
      ...
      .replace(
        /\\\\/g,
        '/'
      )}').then(m=>console.log(JSON.stringify({p:m.parseGCodeToPoints(${JSON.stringify(
      code
    ...
    expect(p[2].type).toBe('rapid'); // G0 is now 'rapid'
  });
  ...
  test('interpolatePoints increases point count for subdivisions > 1', () => {
    ...
    const script = `import('./${require('path')
      .relative(process.cwd(), filepath)
      .replace(
        /\\\\/g,
        '/'
      )}').then(m=>console.log(JSON.stringify({i:m.interpolatePoints([{x:0,y:0,z:0},{x:10,y:0,z:0}],4).length}))).catch(e=>{console.error(e);process.exit(2)})`;
    const { spawnSync } = require('child_process');
    const r = spawnSync('node', ['--input-type=module', '-e', script], { encoding: 'utf8' });
    if (r.status !== 0) {
      console.error(r.stderr);
