...
describe('gcode converter', () => {
  test('convert to 2-digit G-codes', () => {
    ...
    const script = `import('./${path
      ...
      .replace(
        /\\/g,
        '/'
      )}').then(m=>console.log(JSON.stringify({a:m.convertToTwoDigitGCodes('G1 X0 Y0\\nG2 X1 Y1\\nG0 Z5')}))).catch(e=>{console.error(e);process.exit(2)})`;
  ...
  test('convert() dispatches to format', () => {
    ...
    const script = `import('./${path
      ...
      .replace(
        /\\/g,
        '/'
      )}').then(m=>console.log(JSON.stringify({a:m.convert('G1 X0 Y0', { format: '2-digit' })}))).catch(e=>{console.error(e);process.exit(2)})`;
    const r = spawnSync('node', ['--input-type=module', '-e', script], { encoding: 'utf8' });
    if (r.status !== 0) {
      console.error(r.stderr);
      throw new Error('Subprocess failed');
  ...
  test('preserves comments during conversion', () => {
    const filepath = path.resolve(__dirname, '../../../modules/cli/gcode-convert.mjs');
    ...
    const script = `import('./${path
      ...
      .replace(
        /\\/g,
        '/'
      )}').then(m=>console.log(JSON.stringify({a:m.convertToTwoDigitGCodes('${input}')}))).catch(e=>{console.error(e);process.exit(2)})`;
    const r = spawnSync('node', ['--input-type=module', '-e', script], { encoding: 'utf8' });
    if (r.status !== 0) {
      console.error(r.stderr);
      throw new Error('Subprocess failed');
    ...
    const out = JSON.parse(r.stdout.trim());
    expect(out.a).toContain('; Test comment');
    expect(out.a).toContain('(Another comment)');
  });
